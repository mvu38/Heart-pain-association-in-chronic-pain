...
# read data
data <- read.csv("pain study 2 60 days.csv")
...
pubm1pt2 <- lmer(pain ~ averageBPM * paintype_ef + averageHRV * paintype_ef + day_from_start+ pain_yesterday + (1|pp), data=data)
summary(pubm1pt2)
...
bpm_prereg <- lmer(averageBPM ~ day_from_start_norm + pain_yesterday * PK_prop * paintype_ef + (1|pp),data=data)
summary(bpm_prereg)

hrv_prereg <- lmer(averageHRV ~ day_from_start_norm + pain_yesterday * PK_prop * paintype_ef + (1|pp),data=data)
summary(hrv_prereg)
